...
hadeer.findById(req.params.productId).exec(function(err,hadeer) {
    if (err) {
      return next(err);
    }
    if (!hadeer) {
...
module.exports.getProducts = function(req, res, next) {
  hadeer.find({}).exec(function(err, hadeer) {
    if (err) {
      return next(err);
    }
    res.status(200).json({
...
module.exports.createProduct = function(req, res, next) {
  ...
  hadeer.create(req.body, function(err, hadeer) {
    if (err) {
      return next(err);
    }
    res.status(201).json({
...
module.exports.deleteProduct = function(req, res, next) {
  ...
  hadeer.findByIdAndRemove(req.params.productId).exec(function(
    err,
    deletedProduct
  ) {
    if (err) {
